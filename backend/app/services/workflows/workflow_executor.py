...
class WorkflowExecutor:
    ...
    def _create_workflow_agent(self, mode='deep'):
        ...
        """
        from app.core.engines.smolagents import labos_engine
        from app.core.engines.smolagents.agents import create_manager_agent, load_agent_prompts

        # Get global resources from labos_engine
        models = {
            'claude_model': labos_engine.claude_model,
            'gpt_model': labos_engine.gpt_model,
            'manager_model': labos_engine.manager_model
        }

        # Use the same tools and managed agents as the global instance
        manager_tools = labos_engine.manager_tool_management
        ...
        tool_creation_tools = [
            ...
            WebSearchTool(),
            visit_webpage,
            labos_engine.base_tools[2],  # search_github_repositories
            labos_engine.base_tools[3],  # search_github_code
            labos_engine.base_tools[4],  # get_github_repository_info
            labos_engine.base_tools[5],  # check_gpu_status
            labos_engine.base_tools[6],  # create_requirements_file
            labos_engine.base_tools[7],  # monitor_training_logs
        ]
        ...
        # Create dev agent
        all_dev_tools = labos_engine.base_tools + labos_engine.mcp_tools
        ...
        if mode == 'fast':
            ...
            manager_agent = create_manager_agent(
                ...
                use_template=labos_engine.use_templates,
                agent_prompts=agent_prompts,
                mode=mode
            )
