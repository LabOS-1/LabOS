...
# file_id can be: UUID (legacy) or composite path like "project_id/generated/filename.png"
_FILE_ID_PATTERN = re.compile(r'\(file_id:\s*[^\)]+\)')
...
# Match both with and without emoji prefix: "📊 Generated visualization:" or "Generated visualization:"
# file_id format: "project_id/generated/filename.png" or legacy UUID
_NEW_FORMAT_VIZ_PATTERN = re.compile(r'(?:📊\s*)?Generated visualization:\s*([^\s]+\.(?:png|jpg|jpeg|gif|svg|webp))\s*\(file_id:\s*([^\)]+)\)')
...
class WorkflowContext:
    ...
    - ws_callback: WebSocket callback for collecting steps (LangChain V2)
    """

    def __init__(self, workflow_id: str, step_counter: Dict[str, int], ws_callback: Any = None):
        self.workflow_id = workflow_id
        self.step_counter = step_counter  # Shared dict: {'count': N}
        self.metadata: Dict[str, Any] = {}
        self.is_cancelled: bool = False  # Cancellation flag
        self.ws_callback = ws_callback  # Optional WebSocket callback reference

    def __repr__(self):
        return f"WorkflowContext(workflow_id={self.workflow_id}, step={self.step_counter['count']}, cancelled={self.is_cancelled})"


def set_workflow_context(workflow_id: str, step_counter: Dict[str, int], metadata: Optional[Dict[str, Any]] = None, ws_callback: Any = None):
    """
    ...
    Args:
        ...
        ws_callback: Optional WebSocket callback for collecting steps (LangChain V2)
    ...
    """
    context = WorkflowContext(workflow_id, step_counter, ws_callback=ws_callback)
    if metadata:
...
def emit_tool_call_event(tool_name: str, params: Optional[Dict[str, Any]] = None, is_agent: bool = False) -> bool:
    ...
    # Emit event for WebSocket broadcast
    workflow_logger = logging.getLogger('labos.workflow')
...
def emit_visualization_event(
    ...
    """
    Emit a visualization event directly with base64 data.

    This is used by plotting tools to send visualization directly to the frontend
    without needing file_id-based retrieval.

    Args:
        filename: Name of the visualization file
        title: Title of the visualization
        chart_type: Type of chart (line, bar, scatter, etc.)
        base64_data: Full base64-encoded image data (data:image/png;base64,...)
        tool_name: Optional name of the tool that created this
    ...
    context = get_workflow_context()
    if not context:
        print(f"⚠️ Cannot emit visualization event: No workflow context available")
        return False

    # Prepare visualization metadata with base64 for direct display
    visualization_metadata = {
        'visualizations': [{
            ...
            "title": title,
            "filename": filename,
            "base64": base64_data  # Full base64 for direct rendering
        }]
    }
    ...
    event = WorkflowEvent(
        ...
        timestamp=datetime.now(),
        step_number=context.step_counter['count'],
        title=f"📊 Created Visualization: {title}",
        description=f"Generated {chart_type} chart: {filename}",
        tool_name=tool_name or f"create_{chart_type}_plot",
        step_metadata=visualization_metadata
    )

    workflow_event_queue.put(event)
    print(f"📤 Emitted visualization event: {filename} with base64 data ({len(base64_data)} chars)")
