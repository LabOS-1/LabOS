...
OPENROUTER_API_KEY_STRING = os.getenv('OPENROUTER_API_KEY_STRING')
if not OPENROUTER_API_KEY_STRING:
    raise ValueError("OPENROUTER_API_KEY_STRING must be set in environment variables")

# Use absolute path for schema database
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
AGENTS_LABOS_DIR = os.path.dirname(SCRIPT_DIR)
SCHEMA_DB_PATH = os.path.join(AGENTS_LABOS_DIR, "resource", "schema_db")
...
def get_hpo_names(hpo_terms: List[str], ) -> List[str]:
    ...
    """
    hp_dict = parse_hpo_obo(os.path.join(AGENTS_LABOS_DIR, 'resource', 'hp.obo'))
