...
def get_auth0_config():
    ...
    # Get environment-specific URLs from unified config
    current_urls = ENVIRONMENT_URLS.get(ENVIRONMENT, ENVIRONMENT_URLS["development"])
    
    config = {
        'domain': AUTH0_CONFIG["issuer_base_url"],
        'client_id': AUTH0_CONFIG["client_id"],
        'client_secret': AUTH0_CONFIG["client_secret"],
        'base_url': AUTH0_CONFIG["base_url"],  # Frontend URL
        'backend_url': current_urls["backend_url"],  # Backend URL from unified config
...
async def login(request: Request):
    ...
    try:
        ...
        # Override URLs based on request origin if needed (for mixed environments)
        origin = request.headers.get('origin') or request.headers.get('referer')
        if origin and config['environment'] == 'development':
            if 'localhost:3000' in origin or '127.0.0.1:3000' in origin:
                # Ensure we use localhost for both frontend and backend in development
                config['base_url'] = 'http://localhost:3000'
                config['backend_url'] = 'http://localhost:18800'
                logger.info(f"🔄 Auth0: Using localhost URLs for development origin: {origin}")
            else:
                logger.info(f"🔄 Auth0: Using default development URLs for origin: {origin}")
        elif origin:
            logger.info(f"🔄 Auth0: Using environment-based URLs for {config['environment']} with origin: {origin}")
        
        # Build callback URL pointing to backend
        callback_url = f"{config['backend_url']}/api/auth/callback"
...
async def logout(request: Request):
    ...
    try:
        ...
        # Override URLs based on request origin if needed (same logic as login)
        origin = request.headers.get('origin') or request.headers.get('referer')
        if origin and config['environment'] == 'development':
            if 'localhost:3000' in origin or '127.0.0.1:3000' in origin:
                config['base_url'] = 'http://localhost:3000'
                logger.info(f"🔄 Auth0 logout: Using localhost URL for development origin: {origin}")
            else:
                logger.info(f"🔄 Auth0 logout: Using default development URLs for origin: {origin}")
        elif origin:
            logger.info(f"🔄 Auth0 logout: Using environment-based URLs for {config['environment']} with origin: {origin}")
...
async def submit_waitlist_application(request: Request, db: AsyncSession = Depends(get_db_session)):
    ...
    try:
        ...
        admin_email = os.getenv('ADMIN_EMAIL', 'gonbik7@gmail.com')
