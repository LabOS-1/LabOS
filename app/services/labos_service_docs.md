...
This document helps navigate `stella_service.py` (985 lines).
...
- `_initialize_stella_direct()` - Direct initialization (77-99)
...
- `_process_with_stella()` - Main workflow executor (293-592)
...
| **Execute workflow** | `_process_with_stella()` | 293-592 |
...
### 1. Workflow Execution (_process_with_stella)
...
process_message_async() ← WebSocket entry
    ↓
_process_with_stella() ← Core execution
    ↓
